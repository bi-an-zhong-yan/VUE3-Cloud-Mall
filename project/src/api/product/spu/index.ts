...
enum API {
    ...
    //获取全部品牌的数据
    ALLTRADEMARK_UEL = '/admin/product/baseTrademark/getTrademarkList',
    //某一个spu下全部的照片商品数据
...
//获取全部的SPU品牌的数据
export const reqAllTradeMark = () => request.get<any, AllTradeMark>(API.ALLTRADEMARK_UEL);
//获取某一个已有spu下全部商品的数据
export const reqSpuImageList = (spuId: number) => request.get<any, SpuHasImg>(API.IMAGE_URL + spuId)
//获取每一个已有的SPU拥有多少个已有的销售属性
